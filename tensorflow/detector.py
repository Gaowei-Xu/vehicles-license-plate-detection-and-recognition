...
class CNLicensePlateDetector(object):
    def __init__(self, model_root_path, enable_vis=True):
        self._model_root_path = model_root_path
        self._enable_vis = enable_vis
    ...
    def detect(self, image):
        ...
        if self._enable_vis:
            self.visualize(
                image=image,
                bbox_coords=boxes, bbox_scores=scores, class_ids=cls_ids, class_names=['license plate'])

        return boxes, scores

    @staticmethod
    def visualize(image, bbox_coords, bbox_scores, class_ids, class_names):
        image = image[:, :, ::-1]
        ax = utils.viz.plot_bbox(
            ...
            linewidth=1.5)
        plt.axis('off')
        plt.savefig('license-plate-detection.jpg', dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close()
